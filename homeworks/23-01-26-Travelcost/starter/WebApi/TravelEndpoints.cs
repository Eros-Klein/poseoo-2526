...
public static class TravelEndpoints
{
    public static IEndpointRouteBuilder MapTravelEndpoints(this IEndpointRouteBuilder app)
    {
        ...
        // List all travels (summary only)
        group.MapGet("/", GetTravelList)
            .Produces<List<TravelListItemDto>>(StatusCodes.Status200OK)
            .WithName("ListAllTravels")
            .WithDescription("Returns a list of all travels (summary information only).");

        // Get details for a single travel
        group.MapGet("/travels/{id:int}", GetTravelDetails)
            .Produces<TravelDetailsDto>(StatusCodes.Status200OK)
            .Produces(StatusCodes.Status404NotFound)
            .WithName("GetDetail")
            .WithDescription("Returns detailed information about a single travel, including reimbursements.");
    ...
    /// <summary>
    /// Returns a list of all travels (id, name, purpose only).
    /// </summary>
    private static async Task<IResult> GetTravelList(ApplicationDataContext db)
    {
        var items = await db.Travels
            .AsNoTracking()
    ...
    /// <summary>
    /// Returns details about a single travel including its reimbursements.
    /// </summary>
    private static async Task<IResult> GetTravelDetails(int id, ApplicationDataContext db)
    {
        var entity = await db.Travels
            .AsNoTracking()
        ...
        return Results.Ok(MapToDetailsDto(entity));
    }
