...
public class TimesheetImportDatabaseReaderWriter(ApplicationDataContext context) : ITimesheetImportDatabaseReaderWriter
{
    ...
    {
        await context.TimeEntries.Include(te => te.Employee).Where(te => te.Employee != null && te.Employee.EmplyeeId.Equals(employeeId) && te.Date == date).ExecuteDeleteAsync();
    }

    public async Task WriteTimeEntriesAsync(IEnumerable<TimeEntry> entries)
    {
        await context.TimeEntries.AddRangeAsync(entries);
